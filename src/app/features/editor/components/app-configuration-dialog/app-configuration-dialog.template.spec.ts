...
describe('AppConfigurationDialogComponent template', () => {
  ...
  it('uses a compact behavior grid and exposes context-menu configuration', async () => {
    ...
    expect(template).toContain('@for (row of contextMenuActionDescriptors; track row.action)');
    expect(template).toContain('[iconPath]="row.iconPath"');
    expect(template).not.toContain('draggable="true"');
    expect(template).not.toContain('context-menu-action-order');
    expect(template).not.toContain('startContextMenuActionDrag');
  });
